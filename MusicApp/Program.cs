...
namespace MusicApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nMaria favorite songs are:\n");
            Maria.GetFavoritSongs();
            Jane.FavoriteSongs = Songs.Where(song => song.Genre == Genre.Rock).ToList();
            Console.WriteLine("\nJane favorite songs are:\n");
            Jane.GetFavoritSongs();
            Stefan.FavoriteSongs = Songs.Where(song => song.Genre == Genre.Hip_Hop && song.Length < 180).ToList();
            Console.WriteLine("\nStefan favorite songs are:\n");
            Stefan.GetFavoritSongs();
            var musicLovers = FansArray.Where(fan => fan.FavoriteSongs.Count >= 4).ToList();
            Console.WriteLine("\nPeople who are music lovers are:\n");
            foreach (var person in musicLovers)
            {
